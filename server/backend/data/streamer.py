...
BACKEND_SERVER = (
    f"{os.getenv('BACKEND_HOST', 'localhost')}:{os.getenv('BACKEND_PORT', '4595')}"
)


def post_to_endpoint():
    requests.post(
        f"{BACKEND_SERVER}/api/v1/something",
        json=json_gz_to_df(f"{DATA_DIR}/{BATCH_FILES[0]}").to_json(),
    )


def get_amzn_reviews(field_list, metadata=True):
    if metadata:
        partitions = [filename for filename in BATCH_FILES if "review" not in filename]
    else:
        partitions = [
            filename for filename in BATCH_FILES if "metadata" not in filename
        ]
    print(partitions[0])
